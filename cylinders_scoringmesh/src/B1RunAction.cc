...
extern G4double Electric_field_z;
...
//  ss << setprecision(4) << num << endl;
  ...
  convert_field << Electric_field_z / (keV/cm);      // insert the textual representation of 'Number' in the characters in the stream
...
//  G4String about_angle (", angle = ");
  ...
  G4String title_preliminary = physical_value_and_field + this_string_is_field + unit_of_Energy + about_distance +
          this_string_is_distance + unit_of_distance +
...
//  G4double diff_V =  Electric_field_z * from_start_to_the_detector;
...
{ 
  ...
  // Open an output file
  //
  G4String fileName = "cylinder_output_hist";
